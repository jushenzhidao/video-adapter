...
⚠️ **限额只声明在查询接口上**：创建（`POST /api/v1/generate/{model}`）与取消（`PUT …/cancel`）
没有被列入 ⇒ 本层**只对查询计时**（创建 / 取消不占用配额），不替上游扩大解释。
但它们收到 429 时**仍会回灌冷却** —— 429 意味着"这个出口 IP 已经被盯上"，对查询同样成立。

### 6.1 三个必须自己推出来的推论

文档只给了一句话，但"**按 IP**"这个限定词本身就决定了实现形状。三条推论各自对应一个
**具体的错误做法**：

| # | 推论 | 正确做法 | 错误做法的后果 |
| --- | --- | --- | --- |
| 1 | 配额的计量主体是**出口 IP**，不是 Key、不是 `provider` | 限速桶的键取**上游 origin**（`scheme://host:port`） | 按 `provider` 分桶 ⇒ 两条渠道各拿 60，合起来仍发 120/min，照样被拒 |
| 2 | 配额是**共享**的（同一出口 IP 上还有别的消费者，本层看不到别人用了多少） | **主动只声明一部分**（默认 30/min，见 §6.3） | 贴着 60 跑 ⇒ 与别人的流量叠加后必然超限，表现为"一直在 429" |
| 3 | 429 是**全局**信号：踩线的不是那一个任务，而是整个出口 | 任一请求收到 429 ⇒ 冷却该 origin 的**所有**后续查询 | 只让触发它的请求退避 ⇒ 同 origin 的其他查询继续撞墙 |

### 6.2 `Retry-After` 的两种合法格式

文档只说"响应头包含 `Retry-After`"，**没说是 delta-seconds 还是 HTTP-date**
（RFC 9110 两者都合法）⇒ 本层两种都解析；都解析不出时**不编数字**（一律记 warning）。

- 上游给了值 ⇒ **照传**给调用方，不改写（本地实际冷却可能因上限而短于它，那是本地口径）；
- 上游没给 ⇒ 记 warning（"上游没说"必须可见），本地按 1s 兜底冷却。

### 6.3 本层采取的口径（实现见 `docs/03_引擎架构.md` §7.1）

| 项 | 取值 | 为什么 |
| --- | --- | --- |
| 默认配额 | **30/min**（上游 60 的一半） | 推论 2：给同一出口的其它消费者留余量 |
| 突发额度 | 5 | 上游的窗口实现未知；固定窗口在边界会放行双倍，一次放出 60 个是"必然踩线"的写法 |
| 无令牌时 | 排队 ≤3s，超预算 ⇒ 429 + `Retry-After` | 契约内语义（429 本来就是要调用方退避） |
| 收到 429 时 | 设**全局冷却**（按 `Retry-After`，超 120s 截断并**告警**） | 推论 3 |
| 冷却期内 | **不排队**，直接 429 | 上游已明确说"别来了"；排队只会把连接堆在这里，等到的仍然是 429 |
| 429 原地重试 | 仅当 `Retry-After ≤ 1s` | 窗口刚开始滚动时能透明恢复；大值原地等无收益且占连接 |
| 降频前置 | 短 TTL 缓存 + single-flight 合并 | **比退避有效得多**：真正减少上游请求数（§7.1） |

> 🔴 **不要用"疯狂重试"对抗限流**。60/min 的硬顶下，每次 429 再带一次重试会把请求数
> 放大到配额**之上** —— 表现出来就是"永远卡在 429"。上游自己的处理建议也是退避，不是重试。
