...
> ⚠️ **范围校正（2026-09-14，见 `ADR-010`）**：本文的"支出上限"**不是计费** ——
> 计费与配额归属 **new-api**（本服务作为上游渠道挂在它后面）。
> `max_credits` 的定位收敛为**上游账户的安全栏杆**：它防的是"误提交把运营方的上游余额花掉"，
> 不是"向调用方收钱、扣额度"。**本文的事实前提与三道闸门全部仍然有效**，
> 只是不再表述为"由本层承担的资金安全责任"。
...
## Consequences
- **正面**：不可能因为"上游没守"而产生意外账单。⚠️ 但这是**上游账户栏杆**、不是计费
  （计费在 new-api，见 `ADR-010`）：它防的是把**运营方的上游余额**花掉，不是向调用方收钱。
